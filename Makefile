...
check:
	...
	$(MISC) clrty/huggingface-sync-bridge.mis --check --compact-letters
	$(MISC) mis/clrty1_e2e_bind.mis --check --compact-letters
	$(MISC) mis/kernel/HuggingfaceSyncBridgeKernel.mis --check --compact-letters
	@find mis -name '*.mis' | while read f; do $(MISC) "$$f" --check --compact-letters || exit 1; done
	@echo OK huggingface-sync-bridge
...
network-connect: commands-check
	...
	@echo MODULES mis/commands/HuggingfaceSyncBridgeCommands.mis
